...
int StudentWorld::init()	//loads current level and creates inital actors
{
    m_bonus = 1000;
    m_nextLev = false;
    ...
    for (int x = 0; x < VIEW_WIDTH; x++) //put in characters on level
    {
...
void StudentWorld::cleanUp()	//to deallocate memory when level over
{
    delete m_player;
    for (int i = m_actors.size(); i > 0; i--)
    {
        delete m_actors[i - 1];
