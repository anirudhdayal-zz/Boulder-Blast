...
:Entity(startID, startX, startY, world, health, dir), m_ticks(0), m_scoreAward(score)
{
...
void Factory::doSomething()	//if < 3 Kleptobots in 3x3 box around this and no Kleptobots are on the same space as this, has a 1/50 chance of spawning a Kleptobot, depending on its type (angry/normal)
{
    vector<Actor*>* actors = getWorld()->getActors();
    int kleptoKount = 0;
...
void AngryKlepto::doSomething()	//also attempts to shoot before attempting to steal or move
{
    if (!isAlive())
        return;
